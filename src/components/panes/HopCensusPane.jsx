...
function HopCensusPane({ data, networkName }) {
  ...
  useEffect(() => {
    ...
    setFilter((event) => {
      ...
      if (
        ...
        event.target.classList.contains("census-line")
      )
        return false;
      return true;
    });
  ...
  useEffect(() => {
    if (
      !svgRef.current ||
      !vectorsByIdxRef.current ||
      !bandIndexMapRef.current ||
  ...
  const initializeVisualization = useCallback(() => {
    ...
    const sorted = [...data.census_vectors].sort(
      (a, b) => b.vector_length - a.vector_length,
    );
    ...
    const xScale = d3.scaleLinear().domain([0, maxHop]).range([0, innerWidth]);
    ...
    vectorsByIdxRef.current = new Map(
      data.census_vectors.map((v) => [v.node_idx, v.vector]),
    );
    ...
    const contentGroup = zoomContainer
      ...
      .attr(
        "transform",
        `translate(${margin.left},${margin.top + preOffsetY})`,
      );
