...
function KSnakesPane({ data, nodeLinkData, networkName }) {
  ...
  useEffect(() => {
    ...
    setFilter((event) => {
      ...
      if (
        ...
        event.target.classList.contains("snake-node")
      )
        return false;
      return true;
    });
  ...
  const initializeVisualization = useCallback(() => {
    ...
    // (clip path removed)
    svg.append("defs").attr("height", innerHeight);
    ...
    const contentGroup = zoomContainer
      .append("g")
      .attr("class", "content")
      .attr(
        "transform",
    ...
    data.cores.forEach((core) => {
      ...
      // Sort by x_position
      const sortedNodes = [...coreNodes].sort(
        (a, b) => a.x_position - b.x_position,
      );
    ...
    data.cores.forEach((core) => {
      core.islands.forEach((island) => {
        ...
        // Sort by x_position
        const sortedNodes = [...island.nodes].sort(
          (a, b) => a.x_position - b.x_position,
        );
    ...
    if (nodeLinkData && nodeLinkData.edges) {
      ...
      allNodes.forEach((n) => {
        nodePositions.set(n.node_idx, {
          x: xScale(n.x_position),
          y: yScale(n.onion_value),
        });
      });

      const edgeOverlayGroup = contentGroup
        .append("g")
      ...
      core.islands.forEach((island) => {
        island.nodes.forEach((node) => {
          if (!minNode || node.x_position < minNode.x_position) {
            ...
            node.x_position === minNode.x_position &&
            node.onion_value < minNode.onion_value
          ) {
            minNode = node;
          }
        });
  ...
  useEffect(() => {
    ...
    svg
      .selectAll(".island-singleton")
      .attr("r", structureSizes.islandSingleton);
